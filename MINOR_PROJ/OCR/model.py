...
def train_model(input_dimen, output_dimen, activation='leaky_relu', dropout=0.2):
    
    inputs = layers.Input(shape=input_dimen, name="input")        

    input = layers.Lambda(lambda x: x / 255)(inputs)            # normalization
    ...
    squeezed_req = layers.Reshape((x9.shape[-3] * x9.shape[-2], x9.shape[-1]))(x9)

    blstm = layers.Bidirectional(layers.LSTM(64, return_sequences=True))(squeezed_req)

    output = layers.Dense(output_dimen + 1, activation='softmax', name="output")(blstm)
